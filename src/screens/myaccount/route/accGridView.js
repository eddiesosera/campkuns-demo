...
function AccountGridView({ myPosts }) {
    ...
    return (
        <div style={{ padding: '5px', marginBottom: '60px' }}>
            ...
            <div className='gridposts-wrap' style={{ width: '100%', marginBottom: '0px', display: 'flex', flexWrap: 'wrap', justifyContent: 'flex-start', gap: '2px' }}>
                {/* https://ucarecdn.com/0741b9fc-75ab-464d-b442-a80cb2c1e033/870f608f74c132310fabf55d88328559.jpg */}
                {myPosts.map((myimgs) => { return (imgArr[0].length !== 0 && <img style={{ width: '49.5%', height: '50vw', maxHeight: '228.800px', borderRadius: '3px', objectFit: 'cover' }} className='gridImg' src={myimgs.images[0]} alt='gridImg' />) })

                }{
                    imgArr[0].length === 0 && <div style={{ fontSize: '36px', fontWeight: '500', fontFamily: 'Montserrat', color: '#2b2b2b', margin: '0 auto' }}>No art yet</div>
                }
            </div>
