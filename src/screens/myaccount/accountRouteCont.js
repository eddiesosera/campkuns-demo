...
function AccountRouteCont() {
  const accRoutes = [
    ...
    },
    {
      ...
      active: ["ri-book-open-fill", "#ed6d22"],
      inActive: ["ri-book-line", "#ad9485"]
    },
    {
    ...
    >
      {accRoutes.map(route => (
        <NavLink style={{ textDecoration: "none" }} to={route.path}>
          <div onClick={page}>
            <i
              className={window.location.pathname === route.path ? route.active[0] : route.inActive[0]}
              style={{ fontSize: "20px", color: window.location.pathname === route.path ? route.active[1] : route.inActive[1] }}
            />
          </div>
        </NavLink>
      ))}
