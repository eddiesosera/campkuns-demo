...
function SelectCategory({ display }) {
  ...
  const topBarStyle = {
    ...
    color: "white",
    transition: "top 0.5s cubic-bezier(0.5, 0.55, 0.70, 0.35)",
    display: "flex",
    alignItems: "center",
    position: "fixed",
    top: "0",
  ...
  const filters = [
    ...
    { label: "Explore Various Categories", value: "Explore Various Categories" }
  ];

  const sortings = [
    { label: "All", sortFunction: "function" },
    { label: "Top", sortFunction: "function" },
    { label: "New", sortFunction: "function" }
  ];
    ...
    <>
      <div className="" style={linkStyle}>
        ...
        <div id="cat_pinIcon" style={topBarStyle}>
          <i
            className={toggle ? "ri-pushpin-fill" : "ri-pushpin-line"}
            style={
              toggle
                ? {
                    fontSize: "24px",
      ...
      <div
        className="selectCategory-wrap"
        style={{
          display: showCat,
          backgroundColor: "#151515f7",
          borderTop: "1px solid #303030",
          backdropFilter: "blur(10px)",
          borderRadius: "0 0 15px 15px",
          paddingBottom: "2px",
          position: "fixed",
          transition: "all 0.9s cubic-bezier(0.5, 0.55, 0.70, 0.35)"
        }}
      >
        <div
          className="selectCategory-top-sec"
          style={{
            display: "flex",
            fontFamily: "Hanken Grotesk",
            fontSize: "30px",
        ...
        >
          ...
          <i className="ri-close-line" style={{ color: "#FFE7D9", cursor: "pointer" }} onClick={openCat} />
        </div>
          ...
          >
            ...
            >
              <Link style={{ textDecoration: "none", display: "flex", alignItems: "center", color: "black" }} to="/search">
                ...
                <div style={{ fontSize: "13px", fontWeight: 500, marginLeft: "5px" }}>Search</div>
              </Link>
            </div>
            <ul style={{ padding: "0", margin: "0", display: "flex", gap: "12px", width: "100%", justifyContent: "flex-end" }}>
              {sortings.map(sort => (
                <li
                  key={sort.label}
                  style={sort.label === selectedSort ? actSort : inActSort}
                  onClick={() => setSelectedSort(sort.label)}
                >
                  {sort.label}
                </li>
              ))}
